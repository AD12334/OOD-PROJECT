...
public class HourlyEmployee extends Employee {
  ...
  public HourlyEmployee(String name, String username, String employeeID,
    ...
    Scanner sc =
        new Scanner(new File("OODPROJ/src/mypackage/Hourlyemployeehours/" +
    ...
    // Make this so that the hourly employee constructor reads the
    // hourlyemployee csv and then stores the number of hours they worked in a
    // hashmap
  }

  public void addSubmittable(LocalDate newMonth) { submittable.add(newMonth); }
   ...
   *                    their hours for
   * @param year,       The combination of our month and year gives us the key
   *                    for
   *                    our hashmap
  ...
  public void submithours(String month, int year, String employeeid)
      throws IOException {
    int monthno = Month.valueOf(month.toUpperCase())
                      .getValue(); // Convert month name to an integer
    int targetRow = -1;
    ...
    if (LocalDate.now().isBefore(LocalDate.of(year, monthno, 10)) &&
      ...
      System.out.println("Enter the number of hours worked during week 2 of "
                         + "the working period");
      float hours2 = Float.parseFloat(sc.nextLine());
      System.out.println("Enter the number of hours worked during week 3 of "
                         + "the working period");
      float hours3 = Float.parseFloat(sc.nextLine());
      System.out.println("Enter the number of hours worked during week 4 of "
                         + "the working period");
      ...
      String filePath = "OODPROJ/src/mypackage/Hourlyemployeehours/" +
                        employeeid + "Hours.csv";
      String[] workedValues =
          new String[] {String.valueOf(hours1), String.valueOf(hours2),
      ...
      // Update the appropriate columns in the row assumes data starts from
      // column 1
      for (int j = 0; j < workedValues.length; j++) {
        if (targetRow < csvData.size() &&
            1 + j < csvData.get(targetRow).length) {
          csvData.get(targetRow)[1 + j] =
              workedValues[j]; // Update values starting from column 1
        } else {
      ...
      if (response.equals("Y")) {
        System.out.println("Enter the number of hours worked during week 1 "
                           + "of the working period");
        float hours1 = sc.nextFloat();
        System.out.println("Enter the number of hours worked during week 2 "
                           + "of the working period");
        float hours2 = sc.nextFloat();
        System.out.println("Enter the number of hours worked during week 3 "
                           + "of the working period");
        float hours3 = sc.nextFloat();
        System.out.println("Enter the number of hours worked during week 4 "
                           + "of the working period");
   ...
   * @return this returns the number of hours worked during the first week of
   *         the
   ...
   * @param month, month of the hour
   * @param year,  year of the month
   * @return this returns the number of hours worked during the second week of
   *         the
   *         specified month and year
   */
   ...
   * @param month, month of the hour
   * @param year,  year of the month
   * @return this returns the number of hours worked during the third week of
   *         the
   *         specified month and year
   */
   ...
   * @param month, month of the hour
   * @param year,  year of the month
   * @return this returns the number of hours worked during the final week of
   *         the
   *         specified month and year
   */
   ...
   * therefore estimate their hourly pay by dividing their hourly pay by
   * 52(weeks in a year) and then by 40(work week hours)
   *
